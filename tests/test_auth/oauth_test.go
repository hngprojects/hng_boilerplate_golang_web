...
func TestBeginAuthHandler(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/api/v1/auth/social/faux?provider=faux", nil)
	a.NoError(err)
...
func TestGetAuthURL(t *testing.T) {
	...
	req2, err := http.NewRequest("GET", "/api/v1/auth/social/faux?provider=faux", nil)
	a.NoError(err)
...
func TestCompleteUserAuth(t *testing.T) {
	a := assert.New(t)
	...
	sess := faux.Session{Name: "Rick n' morty", Email: "ricksanchez@example.com"}
	...
	a.Equal(user.Name, "Rick n' morty")
	a.Equal(user.Email, "ricksanchez@example.com")
}

func TestCompleteUserAuthWithSessionDeducedProvider(t *testing.T) {
	a := assert.New(t)
...
func TestStateValidation(t *testing.T) {
	...
	Store = NewProviderStore()
	res := httptest.NewRecorder()
	req, err := http.NewRequest("GET", "/api/v1/auth/social?provider=faux&state=state_REAL", nil)
	a.NoError(err)
	...
	req, _ = http.NewRequest("GET", "/api/v1/auth/social/faux/callback?provider=faux&state=state_REAL", nil)
	session.Save(req, res)
	_, err = CompleteUserAuth(res, req)
	a.NoError(err)

	// Assert that mismatched states will return an error
	req, _ = http.NewRequest("GET", "/api/v1/auth/social/faux/callback?provider=faux&state=state_FAKE", nil)
	session.Save(req, res)
	_, err = CompleteUserAuth(res, req)
	a.Error(err)
}
