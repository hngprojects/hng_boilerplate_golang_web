...
func SetupNewsLetterRoutes(r *gin.Engine, newsController *newsletter.Controller) {
	...
	r.GET("/api/v1/newsletter-subscription", middleware.Authorize(newsController.Db.Postgresql.DB(),
		models.RoleIdentity.SuperAdmin),
		newsController.GetNewsLetters)
	r.DELETE("/api/v1/newsletter-subscription/:id", middleware.Authorize(newsController.Db.Postgresql.DB(),
		models.RoleIdentity.SuperAdmin),
		newsController.DeleteNewsLetter)
	r.GET("/api/v1/newsletter-subscription/deleted",
		middleware.Authorize(newsController.Db.Postgresql.DB(), models.RoleIdentity.SuperAdmin),
