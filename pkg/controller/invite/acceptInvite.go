...
func (base *Controller) PostAcceptInvite(c *gin.Context) {
	...
	err, invitation := invite.AcceptInvitationLink(invitationToken, base.Db.Postgresql)
	if err != nil {
		base.Logger.Error("Failed to accept invitation link", err)
		rd := utility.BuildErrorResponse(http.StatusBadRequest, "error", "Invalid or expired invitation link", err, nil)
		c.JSON(http.StatusBadRequest, rd)
		return
	}
